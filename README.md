...
The Golang bundled tool chain ```go get/build/test...``` comes with a nasty GOPATH environment variable, which must be set before using these tool chain in order to locate the project and dependent libraries. It's quite bored to set that variable again and again among projects even I get everything in project local directory. So this wrapper script is written to help.
...
Put this script on the PATH directory, and use "gov" instead of the bundled Golang command line tool. For example, use
